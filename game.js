...
let gamepadState = {
  ...
  menuFocusIndex: 0,    // Active focused button inside open menus
  lastButtonTime: 0,
...
function renderPlots() {
  ...
  currentPlots.forEach((plot, i) => {
    ...
    // 🎮 Highlight Xbox Controller Focus Ring ONLY when no menus are open
    nodes.plotEl.classList.toggle('gamepad-focused', gamepadState.connected && !activeModal && !isDrawerOpen && gamepadState.selectedPlotIndex === i);
...
// ==========================================================================
// 7. XBOX CONTROLLER ENGINE (FULL GAMEPAD & MENU NAVIGATION)
// ==========================================================================
...
function pollGamepad() {
  ...
  // 1. Title Screen Start (Button A or Start)
  const splashScreen = document.getElementById('splash-screen');
  ...
  // 2. 🔴 UNIVERSAL B BUTTON (Index 1) -> Close ANY open modal or drawer!
  if (gp.buttons[1].pressed) {
    let closedSomething = false;

    const openModals = [shopModal, settingsModal, indexModal, vineModal];
  ...
  // 3. 🕹️ MENU MODE (If Modal or Seed Drawer is Open)
  const activeModal = document.querySelector('.modal.open');
  const isDrawerOpen = seedBagDrawer.classList.contains('open');

  if (activeModal || isDrawerOpen) {
    const container = activeModal || seedBagDrawer;
    const items = Array.from(container.querySelectorAll('button:not([disabled]), .seed-select-card'));
    ...
    if (items.length > 0) {
      ...
      if (focusedItem) {
        ...
        // 🟢 Button A -> Click Focused Item inside Menu!
  ...
  // 4. 🎮 MAIN GAME GRID NAVIGATION (3x3 Garden Plots)
  const axisX = gp.axes[0];
  const axisY = gp.axes[1];
  const dUp = (gp.buttons[12] && gp.buttons[12].pressed) || axisY < -0.5;
  const dDown = (gp.buttons[13] && gp.buttons[13].pressed) || axisY > 0.5;
  ...
  // 5. 🎮 MAIN GAME XBOX BUTTON SHORTCUTS:
  // Button A (Green): Plant / Harvest Focused Plot
  if (gp.buttons[0].pressed) {
    handlePlotClick(gamepadState.selectedPlotIndex, null);
    gamepadState.lastButtonTime = now;
  }

  // Button X (Blue): Toggle Shovel Tool ⛏️
  if (gp.buttons[2].pressed) {
    shovelBtn.click();
    gamepadState.lastButtonTime = now;
  }

  // Button Y (Yellow): Sell All Standard Crops 🤠
  if (gp.buttons[3].pressed) {
    sellBtn.click();
    gamepadState.lastButtonTime = now;
  }

  // Left Bumper LB: Previous Field ◀
  if (gp.buttons[4].pressed) {
    prevFieldBtn.click();
    gamepadState.lastButtonTime = now;
  }

  // Right Bumper RB: Next Field ▶
  if (gp.buttons[5].pressed) {
    nextFieldBtn.click();
    gamepadState.lastButtonTime = now;
  }

  // View Button (Back): Open Seed Bag Drawer 🎒
  if (gp.buttons[8].pressed) {
    seedBagBtn.click();
    gamepadState.lastButtonTime = now;
  }

  // Menu Button (Start): Open Settings Modal ⚙️
